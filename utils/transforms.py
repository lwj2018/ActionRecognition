...
class GroupScale(object):
    """ Rescales the input PIL.Image to exactly
    the given size
    """
...
class To3dFormatTensor(object):
    def __init__(self, div=True, roll=False):
        self.div = div
        self.roll = roll

    def __call__(self, img_group):
        if img_group[0].mode == 'L':
            img = np.stack([np.expand_dims(x, 2) for x in img_group], axis=0)
        elif img_group[0].mode == 'RGB':
            if self.roll:
                img =  np.stack([np.array(x)[:, :, ::-1] for x in img_group], axis=0)
            else:
                img =  np.stack(img_group, axis=0)
        img = torch.from_numpy(img).permute(3,0,1,2).contiguous()
        return img.float().div(255) if self.div else img.float()
